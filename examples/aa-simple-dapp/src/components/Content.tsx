...
import { installPlugin } from "@alchemy/aa-accounts";
import Image from "next/image";
import { useCallback, useState } from "react";
import { Hash, encodeFunctionData } from "viem";

type MintStatus = "Installed" | "Uninstalled" | "Installing" | "Uninstalling";

export default function Hero() {
  const { isLoggedIn, provider } = useWalletContext();
  const [pluginTxHash, setPluginTxHash] = useState<Hash>();
  const [installStatus, setInstallStatus] = useState<MintStatus>("Uninstalled");

  const handleMint = useCallback(async () => {
    if (!provider || !provider.isConnected()) {
      throw new Error("Provider not initialized or connected");
    }
    if (installStatus === "Installing" || installStatus === "Uninstalling") {
      return;
    }
    setInstallStatus(
      installStatus === "Uninstalled" ? "Installing" : "Uninstalling"
    );

    const uoHash = await installPlugin(provider, {
  ...
  return (
    <div className="flex flex-row items-center gap-[64px] max-md:flex-col max-md:text-center">
      ...
      <div className="flex flex-col items-start gap-[48px] max-md:items-center">
        ...
        <div className="text-2xl">Install Session Key Plugin</div>
